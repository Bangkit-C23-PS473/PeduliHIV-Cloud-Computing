...
router.post('/login', (req, res) => {
  ...
  router.post('/createpost', multer.single('photo_header'), imgUpload.uploadToGcs, (req, res) => {
      const { users_username, title, content, tag } = req.body;
      ...
      connection.query(insertQuery, [posts_id, users_username, title, photo_header_id, content], (insertErr, insertResults) => {
        if (insertErr) {
          console.log(insertErr);
  ...
  router.get('/getdoctorconsultations', (req, res) => {
      const { username } = req.query;
